...
/**
 * Get all available data for a session from disk
 */
exports.getSessionData = async (req, res) => {
    try {
        ...
        // Check root recordings directory
        const recordingsDir = path.join(process.cwd(), 'recordings');
        
        if (!fs.existsSync(recordingsDir)) {
             return res.status(200).json({ success: true, sessionId, data: { video: null, audio: null, instructions: [], metadata: null } });
        }

        const allFiles = fs.readdirSync(recordingsDir);
        const fileMap = new Map(); // filename -> fullPath

        allFiles.forEach(f => {
            fileMap.set(f, path.join(recordingsDir, f));
        });

        // 1. Find the latest metadata JSON for this session
        const metadataFiles = allFiles
            .filter(f => f.startsWith(`recording_${sessionId}`) && f.endsWith('.json'))
            .sort((a, b) => {
                const tA = parseInt(a.split('_').pop().replace('.json', '')) || 0;
                const tB = parseInt(b.split('_').pop().replace('.json', '')) || 0;
                return tB - tA; // Newest first
            });

        let sessionData = null;
        if (metadataFiles.length > 0) {
            const content = fs.readFileSync(fileMap.get(metadataFiles[0]), 'utf8');
            sessionData = JSON.parse(content);
        }

        // 2. Look for final or raw video
        let videoData = null;
        const finalFilename = `final_video_${sessionId}.webm`;
        const rawFilename = `recording_${sessionId}_video.webm`;

        if (fileMap.has(finalFilename)) {
            const fullPath = fileMap.get(finalFilename);
            videoData = {
                filename: finalFilename,
                url: `/recordings/${finalFilename}`,
                metadata: sessionData?.viewport ? { viewport: sessionData.viewport } : {},
                isFinal: true,
                receivedAt: fs.statSync(fullPath).mtime
            };
        } else if (fileMap.has(rawFilename)) {
            const fullPath = fileMap.get(rawFilename);
            videoData = {
                filename: rawFilename,
                url: `/recordings/${rawFilename}`,
                metadata: sessionData?.viewport ? { viewport: sessionData.viewport } : {},
                isFinal: false,
                receivedAt: fs.statSync(fullPath).mtime
            };
        ...
        // 3. Look for processed audio
        let audioData = null;
        const audioFiles = allFiles
            .filter(f => f.startsWith(`processed_audio_${sessionId}`) && f.endsWith('.mp3'))
            .sort((a, b) => {
                return fs.statSync(fileMap.get(b)).mtime - fs.statSync(fileMap.get(a)).mtime;
            });

        if (audioFiles.length > 0) {
            const latestAudio = audioFiles[0];
            audioData = {
                filename: latestAudio,
                url: `/recordings/${latestAudio}`,
                text: sessionData?.transcription?.text || "",
                receivedAt: fs.statSync(fileMap.get(latestAudio)).mtime
            };
        } else if (sessionData && sessionData.audioPath) {
            const audioBasename = path.basename(sessionData.audioPath);
            if (fileMap.has(audioBasename)) {
                audioData = {
                    filename: audioBasename,
                    url: `/recordings/${audioBasename}`,
                    text: sessionData.transcription?.text || "",
                    receivedAt: fs.statSync(fileMap.get(audioBasename)).mtime
                };
            }
        }

        // 4. Instructions (use events from sessionData)
        const instructions = sessionData?.events || [];
        ...
        Logger.error("[Frontend Controller] Error fetching session data:", err);
        return res.status(500).json({
            success: false,

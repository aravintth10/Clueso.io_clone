...
if not API_KEY:
    print("[Script Generation] ❌ ERROR: GEMINI_API_KEY not found in environment variables!")
...
OUTPUT RULES:
- ONLY output the script text itself.
- DO NOT include any introductory text, preambles, or conversational filler (e.g., "Here is your script").
- DO NOT include labels like "PRODUCTION-READY SCRIPT:".
- Single continuous paragraph with no newlines.
...
def _clean_script_output(text: str) -> str:
    """Clean and normalize script output, removing AI preambles."""
    if not text:
        return ""
    ...
    preamble_patterns = [
        ...
        r"^here's your clean.*?script.*?:",
        r"^based on the (provided data|transcript).*?:",
        r"^production-ready script:?",
        r"^here is the script.*?:",
        r"^here is the generated script.*?:",
        r"^sure,? here is a script.*?:",
    ]

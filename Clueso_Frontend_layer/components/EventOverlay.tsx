...
interface Props {
    ...
    trimStart?: number; // Offset from start of recording
}
...
export default function EventOverlay({
    ...
    useEffect(() => {
        const trimmedCurrentTimeMs = currentTime * 1000; // Convert to milliseconds
        ...
        const active = events.find(event => {
            ...
            if (rawEventTime < 10000000) { 
                ...
                const startTime = events[0] ? Number(events[0].timestamp) : 0;
                absoluteOffsetMs = rawEventTime - startTime;
            }

            const eventTime = absoluteOffsetMs - (trimStart * 1000);
            const effectEnd = eventTime + (effectDuration * 1000);
            return trimmedCurrentTimeMs >= eventTime && trimmedCurrentTimeMs <= effectEnd;
        });

        setActiveEvent(active || null);
    }, [currentTime, events, effectDuration, trimStart]);
    ...
    return (
        <div className="absolute inset-0 pointer-events-none" style={{ zIndex: 100 }}>
            ...
            >
                ...
                {(activeEvent.type === 'input' || activeEvent.type === 'type') && (
